...
-- Update bookings table to add the missing columns from the migration
ALTER TABLE public.bookings 
ADD COLUMN IF NOT EXISTS total_amount BIGINT,
ADD COLUMN IF NOT EXISTS dp_percentage INTEGER DEFAULT 30,
ADD COLUMN IF NOT EXISTS dp_amount BIGINT,
ADD COLUMN IF NOT EXISTS payment_type TEXT DEFAULT 'booking' CHECK (payment_type IN ('booking', 'direct')),
ADD COLUMN IF NOT EXISTS booking_notes TEXT,
ADD COLUMN IF NOT EXISTS kost_id UUID REFERENCES public.kosts(id) ON DELETE CASCADE;
...
-- Create trigger for payments table updated_at
